export function initBackground() {
  ...
  let particleRgb = document.documentElement.getAttribute('data-theme') === 'light' ? '90,60,35' : '255,255,255';
  window.addEventListener('themechange', (e) => {
    particleRgb = e.detail.theme === 'light' ? '90,60,35' : '255,255,255';
  });
  ...
  function drawBg(){
    ...
    for(let i = 0; i < NODE_COUNT; i++){
      for(let j = i+1; j < NODE_COUNT; j++){
        ...
        if(d < LINK_DIST){
          ...
          ctx.strokeStyle = 'rgba(' + particleRgb + ',' + ((1 - d/LINK_DIST)*LINE_ALPHA) + ')';
          ctx.lineWidth = .9;
          ctx.stroke();
        }
      }
    ...
    for(const n of nodes){
      ...
      ctx.fillStyle = 'rgba(' + particleRgb + ',' + DOT_ALPHA + ')';
      ctx.fill();
    }
